...
```markdown
# PROMPT.md
Write a TypeScript function that prints "Hello, World!"
Save it to hello.ts. The orchestrator will continue iterations until completion.
```

Run with:
```bash
bun run src/index.ts --prompt PROMPT.md --max-iterations 5
```
...
```markdown
# PROMPT.md
Create a TypeScript calculator module with:
- Functions for add, subtract, multiply, divide
- Error handling for division by zero
- JSDoc comments for all functions
- Save to calculator.ts
...
### [Web API](web-api.md)
Create a RESTful API with Hono, including authentication and database integration.
...
```markdown
Create a TypeScript logging utility with:
...
```markdown
Write Vitest tests for the user-auth.ts module:
...
```markdown
Refactor the data-processor.ts file:
...
- The calculateTax() function returns wrong values
...
- Uses only built-in fetch API (no external packages)
- Respects robots.txt
...
1. All tests pass (run: bun test)
2. Code follows ESLint rules (run: bun run lint)
...
Existing files: models.ts, database.ts
...
bun run src/index.ts --prompt examples/simple-task.md
```
...
## Requirements
- Argument parsing with Commander.js or yargs
