...
**Bun**
: A fast all-in-one JavaScript runtime used to run Ralph Orchestrator TypeScript. Provides TypeScript support, package management, and test runner.
...
**Hono**
: Lightweight web framework used for Ralph's monitoring web interface.
...
**Loop Detection**
: Safety feature that detects when an agent is producing repetitive outputs. Uses fuzzy string matching (fuzzball) with a 90% similarity threshold to compare recent outputs and prevent infinite loops.
...
**TypeScript**
: Typed superset of JavaScript used to write Ralph Orchestrator TypeScript. Provides type safety and better tooling.
...
**`.yaml`/`.yml`**
: YAML files used for configuration (e.g., `ralph.yml`).
