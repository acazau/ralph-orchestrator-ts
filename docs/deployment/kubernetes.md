...
Deploy Ralph Orchestrator TypeScript on Kubernetes for scalable, resilient AI orchestration.
...
```bash
# Add repository
helm repo add ralph https://acazau.github.io/ralph-orchestrator-ts/charts
helm repo update

# Install with custom values
helm install ralph ralph/ralph-orchestrator-ts \
  --namespace ralph-orchestrator \
  --create-namespace \
...
image:
  repository: ghcr.io/acazau/ralph-orchestrator-ts
  tag: v1.0.0
...
# Execute into pod
kubectl exec -it <pod-name> -n ralph-orchestrator -- /bin/sh
```
