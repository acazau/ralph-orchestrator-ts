...
The Ralph Wiggum technique was created by [Geoffrey Huntley](https://ghuntley.com/ralph/). This TypeScript implementation builds on his concept with additional features like multiple agent support, checkpointing, and comprehensive testing, adapted for the Bun runtime.

### What AI agents does it support?

Ralph Orchestrator TypeScript currently supports:
...
docker run -v $(pwd):/workspace ralph-orchestrator-ts
```
...
```bash
# Terminal 1
cd project1 && bun run cli

# Terminal 2
cd project2 && bun run cli
```
...
- name: Run Ralph
  run: |
    bun run cli --max-iterations 50 --dry-run
...
Currently, Ralph Orchestrator TypeScript is community-supported open source software. Commercial support may be available in the future.
