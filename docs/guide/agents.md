...
**Installation:**
```bash
# Q CLI installation varies by platform
# Check your platform's package manager
```

**Usage:**
```bash
bun run src/cli.ts --agent q

# Short form
bun run src/cli.ts -a q
```
...
**Usage:**
```bash
bun run src/cli.ts --agent gemini
```
...
bun run src/cli.ts --agent acp --acp-agent gemini --acp-permission-mode auto_approve

# Using allowlist mode
bun run src/cli.ts --agent acp --acp-permission-mode allowlist
```
...
bun run src/cli.ts --agent claude --prompt research.md

# Phase 2: Implementation with Q
bun run src/cli.ts --agent q --prompt implement.md

# Phase 3: Documentation with Claude
bun run src/cli.ts --agent claude --prompt document.md
```
...
```bash
# Try Q first
bun run src/cli.ts --agent q --max-cost 2.0

# If unsuccessful, try Claude
bun run src/cli.ts --agent claude --max-cost 20.0
```
...
bun run src/cli.ts --agent auto --dry-run --verbose
...
bun run src/cli.ts --agent gemini --max-cost 25.0

# High budget: Use Claude
bun run src/cli.ts --agent claude --max-cost 100.0
```
...
You can also work with agents programmatically in TypeScript:

```typescript
import { ClaudeAdapter, GeminiAdapter, QChatAdapter } from 'ralph-orchestrator-ts';
import type { AgentAdapter, AgentResponse } from 'ralph-orchestrator-ts';

// Create adapter instances
const claude = new ClaudeAdapter();
const gemini = new GeminiAdapter();
const qchat = new QChatAdapter();

// Execute a prompt
async function runWithAgent(adapter: AgentAdapter, prompt: string): Promise<AgentResponse> {
  return await adapter.execute(prompt);
}

// Example usage
const response = await runWithAgent(claude, 'Write a hello world function');
...
```bash
bun run src/cli.ts --agent auto --max-iterations 5
```
