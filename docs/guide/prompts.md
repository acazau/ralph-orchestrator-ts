...
Custom location:
```bash
bun run src/cli.ts --prompt path/to/task.md
```
...
```markdown
## Constraints
- Must be TypeScript/Bun compatible
- Cannot use external APIs
- Must complete in under 5 seconds
- Memory usage < 1GB
- Must follow project style guide (Biome)
```
...
Adjust if needed:
```bash
bun run src/cli.ts --max-prompt-size 20971520  # 20MB
```
...
```bash
bun run src/cli.ts --dry-run --prompt test.md
```
...
```bash
bun run src/cli.ts --max-iterations 3 --prompt test.md
```
...
```bash
bun run src/cli.ts --verbose --prompt test.md
```
...
## TypeScript Prompt Handling

You can also work with prompts programmatically:
...
async function parsePrompt(path: string): Promise<PromptSection[]> {
...
async function updatePromptSection(
  path: string,
  sectionTitle: string,
  newContent: string
): Promise<void> {
  ...
  const updated = content.replace(regex, `$1${newContent}\n\n`);
